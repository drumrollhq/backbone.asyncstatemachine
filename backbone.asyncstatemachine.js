  ...
  function (Backbone, _) {
    ...
    var StateMachine = Backbone.StateMachine = {
      ...
      toState: function(name) {
        ...
        return this._callCallbacks(this._states[name].enter, extraArgs)
      },
      ...
      _onMachineEvent: function(event) {
        ...
        if (!(extraArgs.length && extraArgs[extraArgs.length - 1]._stateMachineIgnore)) {
          return this._doTransition.apply(this, [data, event].concat(extraArgs))
        }
      },

      // Trigger a transition, returning a promise that will resolve when the transition is complete
      triggerAsync: function(event) {
        this.trigger.apply(this, _.toArray(arguments).slice().concat([{_stateMachineIgnore: true}]))
        return this._onMachineEvent.apply(this, arguments)
      },
      ...
      _doTransition: function(data, event) {
        ...
        this._lastTransition = lastTransition.then(_.bind(function() {
          ...
          promises = this._callCallbacks(this._states[leaveState].leave, extraArgs)
          ...
          promises = promises.concat(this._callCallbacks(data.callbacks, extraArgs))
          if (!this.silent)
            this.trigger.apply(this, ['enterState:' + enterState].concat(extraArgs))

          promises = promises.concat(this.toState.apply(this, [enterState].concat(extraArgs)))
      ...
      _callCallbacks: function(cbArray, extraArgs) {
        var that = this
        return cbArray.map(function(cb) {
          return Promise.resolve(cb.apply(that, extraArgs))
        })
      }
